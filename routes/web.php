...
Route::get('/refreshs-csrf', function () {
    ...
    foreach ($data as $key=>$item) {
        ...
        $translation_def->lang_value = $item;
        $translation_def->save();
    }

   return $data;
});
Route::get('/load-cities-states', function () {

    $jsonContent = file_get_contents(asset("cities.json"));
    $jsonContents = file_get_contents(asset("districts.json"));
    // Decode JSON content to an associative array
    $data = json_decode($jsonContent, true);
    $datas = json_decode($jsonContents, true);
...
Route::group(['middleware' => ['auth']], function () {
    ...
    Route::get('invoice-image/{order_id}', [InvoiceController::class, 'invoice_download_image'])->name('invoice.image');
