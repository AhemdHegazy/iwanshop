...
function translate($key, $lang = 'ar', $addslashes = false)
{
    $lang_key = preg_replace('/[^A-Za-z0-9\_]/', '', str_replace(' ', '_', strtolower($key)));
    $translations_en = Cache::rememberForever('translations-ar', function () {
...
if (!function_exists('generate_combination')) {
    function generate_combination($arrays, $i=0)
    {
        if (!isset($arrays[$i])) {
            return array();
        }
        ...
        $tmp = generate_combination($arrays, $i + 1);
