                                        ...
                                        <ul class="mb-0 list-unstyled">
                                            @if (!isset($category_id))
                                                @foreach (get_level_zero_categories() as $category)
                                                    <li class="mb-3">
                                                        <a class="text-reset fs-14 hov-text-primary" href="{{ route('customer_products.category', $category->slug) }}">{{ $category->name }}</a>
                                                    </li>
                                                @endforeach
                                            @else
                                                <li class="mb-3">
                                                ...
                                                @if (get_single_category($category_id)->parent_id != 0)
                                                    <li class="mb-3">
                                                        <a class="text-reset fs-14 fw-600 hov-text-primary" href="{{ route('customer_products.category', get_single_category(get_single_category($category_id)->parent_id)->slug) }}">
                                                            <i class="las la-angle-left"></i>
                                                            {{ get_single_category(get_single_category($category_id)->parent_id)->name }}
                                                        </a>
                                                    </li>
                                                @endif
                                                <li class="mb-3">
                                                    <a class="text-reset fs-14 fw-600 hov-text-primary" href="{{ route('customer_products.category', get_single_category($category_id)->slug) }}">
                                                        <i class="las la-angle-left"></i>
                                                        {{get_single_category($category_id)->name }}
                                                    </a>
                                                </li>
                                                @foreach (\App\Utility\CategoryUtility::get_immediate_children_ids($category_id) as $key => $id)
                                                    <li class="ml-4 mb-3">
                                                        <a class="text-reset fs-14 hov-text-primary" href="{{ route('customer_products.category', get_single_category($id)->slug) }}">{{get_single_category($id)->name }}</a>
                                                    </li>
                                                @endforeach
                                            @endif
                                        </ul>
                                        ...
                                        <div class="aiz-card-box my-3">
                                            ...
                                            <div class="p-md-3 p-2 text-center">
                                                ...
                                                <h3 class="fw-400 fs-14 text-truncate-2 lh-1-4 mb-0 h-35px">
                                                    <a href="{{ route('customer.product', $product->slug) }}" class="d-block text-reset hov-text-primary">{{  $product->name  }}</a>
                                                </h3>
                                                <!-- Price -->
