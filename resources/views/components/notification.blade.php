                ...
                <p class="mb-1 text-truncate-2">
                    ...
                    @if ($notification->type == 'App\Notifications\OrderNotification')
                        ...
                        @if ($is_linkable)
                            ...
                            @endphp
                            <a href="{{ $route }}?notify_id={{$notification["id"]}}">
                        @endif
                    ...
                    @elseif ($notification->type == 'App\Notifications\ShopVerificationNotification')
                        @if ($user_type == 'admin')
                            @if ($is_linkable)
                                <a href="{{ route('sellers.show_verification_request', $notification->data['id']) }}?notify_id={{$notification["id"]}}">
                            @endif
                    ...
                    @elseif ($notification->type == 'App\Notifications\ShopProductNotification')
                        ...
                        {{ translate('Product : ') }}
                        @if ($is_linkable)
                            <a href="{{ $route }}?notify_id={{$notification["id"]}}">{{ $product_name }}</a>
                        @else
                            {{ $product_name }}
                        @endif
                    ...
                    @elseif ($notification->type == 'App\Notifications\PayoutNotification')
                        ...
                        @endphp
                         ...
                         @if ($is_linkable )
                             <a href="{{ $route }}?notify_id={{$notification["id"]}}">{{ translate('payment') }}</a>
                         @else
                             {{ translate('payment') }}
                         @endif
