    ...
    public function step0(Request $request)
    {
        if ($request->has('update_zip')) {
            if (class_exists('ZipArchive')) {
                ...
                if (!is_dir($dir))
                    mkdir($dir, 0777, true);
                $path = Upload::find($request->update_zip)->file_name ?? null;
                if(!$path){
                ...
                BusinessSetting::where('type', 'current_version')->first()->update([
                    "value"
                ]);
                flash(translate('Success Updated Successfully'))->success();
            ...
            flash(translate('No File Uploaded.'))->error();
